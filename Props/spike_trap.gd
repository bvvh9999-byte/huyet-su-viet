...
var profile_instance = null # Dùng để quản lý đóng/mở Profile
...
# ==========================================
# HÀM BẮT BÀN PHÍM (LUÔN CHẠY KỂ CẢ KHI PAUSE)
# ==========================================
func _input(event):
	if event.is_action_pressed("open_profile"):
		...
		# Đang mở -> Thu lại
			...
			get_tree().paused = false # Rã đông thế giới

# ==========================================
# HỆ THỐNG VẬT LÝ VÀ CHẠY NHẢY 
# (CHỈ HOẠT ĐỘNG KHI GAME ĐANG CHẠY BÌNH THƯỜNG)
# ==========================================
func _physics_process(delta):
	# NẾU GAME ĐANG PAUSE (MỞ TÚI ĐỒ/LÊN CẤP) THÌ KHÔNG CHO CHẠY NHẢY NỮA
	if get_tree().paused:
		return 
...
func attack():
	...
	# Đợi 1 frame vật lý để Godot nạp vùng chạm
...
func _on_sword_hit_something(body):
	# Hàm phụ trợ này tạm tắt vì đã quét toàn bộ trong hàm attack()
	pass
